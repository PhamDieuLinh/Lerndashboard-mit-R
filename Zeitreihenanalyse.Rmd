...
Stell dir vor, du planst mit zwei Freund:innen eine Reise nach Asien für 2 Monate. Jeder hat ein Reiseziel vorgeschlagen und zur Auswahl stehen:
...
## Zeitreihe
Eine Zeitreihe ist eine Sammlung von Beobachtungen, die fortlaufend über die Zeit hinweg erhoben wurden (Chatfield & Xing, 2019).
Die Zeitreihenanalyse ist ein spezieller Ansatz zur Untersuchung von Daten, die über einen bestimmten Zeitraum hinweg in gleichmäßigen Abständen erhoben wurden (Jose, 2022).
...
Ein häufig auftretendes Problem bei Zeitreihendaten sind fehlende Werte. Sie (Ahn et al., 2021):
...
Dadurch kann die Qualität von Analysen und Prognosen erheblich beeinträchtigt werden (Ahn et al., 2021).

___
FALL 1: fehlende Werte

Die folgende Zeitreihe zeigt die durchschnittliche Temperatur (in °C) von Januar 2024 bis Mai 2025 in Hanoi und enthält bewusst fehlende Werte (NA). 
...
Zu den einfachen Methoden gehören das Ignorieren der fehlenden Werte, das Löschen unvollständiger Beobachtungen (Pratama et al., 2016).
...
question("Was passiert, wenn du NA-Zeilen einfach aus einer Zeitreihe entfernst?",
    answer("Es kann zu Verzerrungen in der Analyse führen", correct = TRUE,
         message = "Richtig! Ignorieren und Löschen zeigen sich oft als ungeeignet, da sie zu Verlusten an Informationsgehalt und einer Verschlechterung der Analyseergebnisse führen können (Pratama et al., 2016)."),
...
Eine praktikable Lösung stellt die sogenannte Mittelwertimputation dar. Fehlende Werte werden durch einen Gesamtmittelwert ersetzt (Pratama et al., 2016).
...
```{r na_exercise-hint}
hanoi_filled <- na_mean(hanoi_mit_na$...)
```


```{r na_exercise-solution}
hanoi_filled <- na_mean(hanoi_mit_na$temperature)
```
...
question("Was hältst du von der Imputation mit `na_mean()` für Temperatur-Zeitreihen?",
  ...
  answer("Sie ist ungeeignet, weil sie keine Trends oder Saisonalität berücksichtigt.", correct = TRUE, message = "Richtig! `na_mean()` setzt für alle fehlenden Werte denselben Mittelwert ein. Dadurch werden die zeitreihenspezifischen Eigenschaften gestört: die eingefügten Werte folgen weder dem Trend noch den saisonalen Mustern und haben oft alle denselben y-Wert. Das kann die Analyse oder Vorhersage erheblich verfälschen  (Moritz et al., 2015)."),
...
na_mean() (Moritz et al., 2022) und (Moritz et al., 2015):
...
Eine andere Methode ist lineare Interpolation. Im ersten Schritt wird die saisonale Komponente von der Zeitreihe getrennt. Anschließend erfolgt die Imputation auf der verbleibenden (nicht-saisonalen) Komponente mittels linearer Interpolation. Danach wird die saisonale Komponente wieder hinzugefügt. Diese Methode eignet sich besonders gut für Zeitreihen mit klarer und ausgeprägter Saisonalität (Moritz et al., 2015).

Die Funktion na_interpolation gehört jedoch zu den Ansätzen, die in der Regel sehr gute Ergebnisse liefern und sich besonders dann eignen, wenn eine genauere Ersetzung fehlender Werte erforderlich ist (Moritz & Bartz-Beielstein, 2017).
...
output$zeitna_hanoi <- renderPlotly({
  p <- ggplot(hanoi_mit_zeitna, aes(x = date, y = temperature)) +
    geom_line() +
    labs(title = "Temperatur-Zeitreihe mit fehlenden Zeitstempeln",
         x = "Datum", y = "Temperatur (°C)") +
    theme_minimal()
...
In R kann man mit dem Paket tibble (Müller & Wickham, 2025) eine vollständige Zeitachse erstellen (z.B. mit seq() für tägliche Daten) und diese anschließend per left_join() mit der ursprünglichen Zeitreihe verbinden. 

Führe einfach den folgenden Code aus, um zu sehen, wie die fehlenden Zeitpunkte in der Zeitreihe ergänzt wurden.
...
Zeitreihen bestehen oft aus folgenden Komponenten (Dagum & Bianconcini, 2016):
...
Die dargestellten Daten zeigen stündlich gemessene E10-Benzinpreise an einer ausgewählten Tankstelle über einen bestimmten Zeitraum. In diesem Plot wird ein Ausschnitt von Woche 3 bis Woche 4 nach Beginn der Aufzeichnung betrachtet. Der Trend zeigt steigende Preisniveaus über den Zeitraum von zwei Wochen. Die Saisonalität bildet regelmäßige, sich täglich wiederholende Schwankungen ab.

## Trendbestimmung
- Gleitende Durchschnitt (Moving Average):

Eine der einfachsten Methoden zur Glättung von Zeitreihen besteht in der Berechnung sogenannter gleitender Durchschnitte. Dabei wird versucht, den zugrunde liegenden Trend einer Zeitreihe zu schätzen, indem man lokale Mittelwerte bildet (Fahrmeir et al., 2006).

Ein gleitender Durchschnitt der Ordnung $m$ wird folgendermaßen definiert (Hyndman & Athanasopoulos, 2021):

$$
\hat{T}_t = \frac{1}{m} \sum_{j = -k}^{k} y_{t + j}, \quad \text{mit } m = 2k + 1 \
$$

Dabei wird der Wert der Trend zum Zeitpunkt $t$ als Durchschnitt der Zeitreihenwerte innerhalb eines symmetrischen Fensters von $k$ Perioden um $t$ berechnet. Da benachbarte Zeitpunkte meist ähnliche Werte aufweisen, werden durch diesen Durchschnitt zufällige Schwankungen reduziert. Übrig bleibt eine geglättete Darstellung des langfristigen Trends (Hyndman & Athanasopoulos, 2021).  
...
ist eine Methode zur Trendglättung, bei der nicht der Durchschnitt benachbarter Punkte berechnet wird, sondern lokale Geraden an Abschnitte der Zeitreihe angepasst werden (Makridakis et al., 1998, S.104-106).

Ein bekanntes Verfahren dafür ist Loess und basiert auf lokaler linearer Glättung, ist jedoch so konzipiert, dass Ausreißer oder ungewöhnliche Beobachtungen weniger Einfluss auf die Trendlinie haben. Dadurch liefert die Loess-Kurve eine robustere Darstellung des Trends, selbst bei untypischen Datenpunkten (Makridakis et al., 1998, S.104-106).
...
In einer Zeitreihe beschreibt die Saisonalität regelmäßig wiederkehrende Muster. Die Länge der Saisonalität gibt an, nach wie vielen Zeitpunkten sich dieses Muster wiederholt (Hyndman & Athanasopoulos, 2021).
...
output$benzin_plot_2 <- renderPlotly({
  ...
  seasonal_subset <- seasonal[1:(7 * 24)]  
  ...
  df <- all_temp %>%
    ...
    mutate(
      ...
      quarter_group = factor(case_when(
        ...
        month %% 4 == 3 ~ "M3-7-11",
        month %% 4 == 0 ~ "M4-8-12" 
      ), levels = c("M1-5-9", "M2-6-10", "M3-7-11", "M4-8-12"))
    )
...
Im klassischen **additiven Modell** setzen sich die Bestandteile der Zeitreihe unabhängig voneinander zusammen. Die folgende Darstellung basiert auf Dagum und Bianconcini (2016):
...
Wenn sich die Komponenten **gegenseitig beeinflussen**, ist ein **multiplikatives Modell** angemessener (Dagum & Bianconcini, 2016):
...
❗ Grenzen der klassischen Zerlegung (Hyndman & Athanasopoulos, 2021):
...
Die **STL-Zerlegung** ist eine moderne, flexible und robuste Methode zur Zerlegung von Zeitreihen (Hyndman & Athanasopoulos, 2021).  
STL steht für:
...
Eine multiplikative Zerlegung lässt sich durch Log-Transformation der Daten und anschließende Rücktransformation der Komponenten durchführen. Zwischenformen von additiven und multiplikativen Zerlegungen können mithilfe der Box-Cox-Transformation realisiert werden (Hyndman & Athanasopoulos, 2021).


## Auto-Korrelation

In Zeitreihen ist es häufig so, dass der heutige Wert mit vorherigen Werten zusammenhängt. Autokorrelation misst die lineare Beziehung zwischen zeitlich versetzten Werten einer Zeitreihe. Hohe Autokorrelationswerte bei bestimmten Lags können auf saisonale Muster oder wiederkehrende Strukturen in den Daten hinweisen (Hyndman & Athanasopoulos, 2021).
...
Zu jedem Lag (also jeder zeitlichen Verschiebung) einer Zeitreihe gehört ein Autokorrelationskoeffizient. Diese einzelnen Koeffizienten ergeben zusammen die Autokorrelationsfunktion (ACF), die angibt, wie stark eine Zeitreihe mit ihren verzögerten Versionen zusammenhängt (Hyndman & Athanasopoulos, 2021).
...
Eine Person ist empfindlich gegenüber starker Hitze, möchte also weder extreme Hitzespitzen noch große Temperaturschwankungen.
Die andere Person mag eher warmes Wetter, möchte aber nicht in eine kalte oder regnerische Jahreszeit reisen.

Nach Analyse der Zeitreihendaten für alle drei Städte:

🌆 Hanoi: Deutlich saisonale Schwankungen mit heißem Sommer, im Frühling und Herbst angenehmer.

🗼 Tokyo: Größte Temperaturschwankungen über das Jahr.

🏝 Manila: Sehr konstante Temperaturen, aber auf durchgehend hohem Niveau und für Hitzeempfindliche eher ungeeignet.

🔍 **Empfehlung:** 
Hanoi im Zeitraum März–April:

- Der langfristige Trend steigt nur leicht.

- Die heißeste Periode (saisonale Spitze) liegt in den Monaten Juni bis August.

- Temperaturschwankungen in Hanoi sind zwar vorhanden, aber deutlich geringer als in Tokyo.

- In diesen Monaten liegen die Temperaturen meist zwischen 21 und 27°C, also über 20°C, weder zu heiß noch zu kalt.
...
| 📊 **Autokorrelation**      | Abhängigkeit eines Werts von vorherigen Zeitpunkten                  |
| ❗ **Umgang mit NAs**        | Fehlende Werte identifizieren und sinnvoll ersetzen  |
...
Danke, dass du die Anwendung genutzt hast!

Dein Feedback ist sehr wichtig! Nimm dir bitte kurz Zeit und fülle die Umfrage aus! 

[👉 Jetzt Umfrage ausfüllen](https://docs.google.com/forms/d/e/1FAIpQLSfkOXPH_UW_TBGEg7UlM7JR5-tzE-JoCKfqcDT04OejclnmNQ/viewform?usp=dialog)


## Quellen

Ahn, H., Sun, K. & Kim, K. P. (2021). Comparison of Missing Data Imputation Methods in Time Series Forecasting. Computers, Materials & Continua/Computers, Materials & Continua (Print), 70(1), 767–779. https://doi.org/10.32604/cmc.2022.019369

Chatfield, C. & Xing, H. (2019). The Analysis of Time Series: An Introduction with R. https://www.amazon.com/Analysis-Time-Introduction-Chapman-Statistical/dp/1138066133

Dagum, E. B. & Bianconcini, S. (2016). Seasonal Adjustment Methods and Real Time Trend-Cycle Estimation. Springer.

Fahrmeir, L., Pigeot, I. & Tutz, G. (2006). Statistik: Der Weg zur Datenanalyse. Springer-Verlag.

Hyndman, R.J., & Athanasopoulos, G. (2021) Forecasting: principles and practice, 3rd edition, OTexts: Melbourne, Australia. OTexts.com/fpp3.

Jose, Jonath. (2022). INTRODUCTION TO TIME SERIES ANALYSIS AND ITS APPLICATIONS. https://www.researchgate.net/publication/362389180_INTRODUCTION_TO_TIME_SERIES_ANALYSIS_AND_ITS_APPLICATIONS

Makridakis, S. G., Wheelwright, S. C. & Hyndman, R. J. (1998). Forecasting: Methods and Applications.

Moritz S, Sarda A,  Bartz-Beielstein T, Zaefferer M, Stork J (2015). Comparison of different Methods for Univariate Time Series Imputation in R. https://arxiv.org/pdf/1510.03924
...
Moritz, S., Gatscha, S., Wang, E. & Hause, R. (2022). Time Series Missing Value Imputation. Package „imputeTS“. In The Comprehensive R Archive Network (Bd. 9, Nummer 1, S. 207). https://doi.org/10.32614/rj-2017-009

Müller K, Wickham H (2025). tibble: Simple Data Frames. R package version 3.3.0, https://tibble.tidyverse.org/.

Pratama, I., Permanasari, A. E., Ardiyanto, I. & Indrayani, R. (o. D.). A review of missing values handling methods on time-series data. 2016 International Conference On Information Technology Systems And Innovation (ICITSI), Bandung, Indonesia. https://ieeexplore.ieee.org/stamp/stamp.jsp?tp=&arnumber=7858189

Datensätze aus meteostat.net und tankerkoenig.de
